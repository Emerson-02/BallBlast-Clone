...
public class CannonScript : MonoBehaviour
{
    ...
    void Update()
    {
        if(canMove)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                Time.timeScale = normalTimeScale; // Restaurar o tempo normal
            }

            if (Input.GetMouseButtonUp(0))
            {
                isDragging = false;
                Time.timeScale = slowTimeScale; // Desacelerar o tempo
            }
    ...
    private void OnCollisionEnter2D(Collision2D other) {
        if (other.gameObject.CompareTag("Jewel"))
        {
            ...
            // Mostrar o painel de derrota
            uiController.panelLose.SetActive(true);
        }
    }
